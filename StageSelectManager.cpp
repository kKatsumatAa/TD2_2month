...
void StageSelectManager::Draw(CameraManager* cameraM)
{
	...
	object[selectNum].worldMat->scale = { stageImageRadius * 1.2f + sinf(count * 0.1f),stageImageRadius * 1.2f + sinf(count * 0.1f) ,stageImageRadius * 1.2f };

	for (int i = 0; i < this->selectNumMax; i++)
	{
		if (object[i].worldMat->scale.x != stageImageRadius * 1.2f && selectNum != i)
		{
			object[i].worldMat->scale = { stageImageRadius * 1.2f ,stageImageRadius * 1.2f ,stageImageRadius * 1.2f };
		}

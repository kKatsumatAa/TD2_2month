...
void Block::Draw(Camera* camera, UINT64* texhandle, int form, Action action)
{
	...
	if (form == Form::GOAL) { draw[3].DrawModel(&worldTransform_, &camera->viewMat, &camera->projectionMat, &goal_[0]); }

	if (form == Form::LOCKED) { draw[2].DrawModel(&worldTransform_, &camera->viewMat, &camera->projectionMat, &socket_[0]); }
